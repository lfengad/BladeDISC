...
_ROOT = "/global/home/aliliang/tao_built"
...
#    session_config.graph_options.rewrite_options.auto_mixed_precision = 1
    ...
    l = 384
    feed_dict = {
            'input_ids_1:0' : np.ones((1, l), dtype=int),
            'segment_ids_1:0' : np.zeros((1, l), dtype=int),
            'input_mask_1:0' : np.ones((1, l), dtype=int),
            }
    for i in range(50):
    ...
    for batch in targets:
        feed_dict = {
'input_ids_1:0' : np.ones((batch, l), dtype=int),
'segment_ids_1:0' : np.zeros((batch, l), dtype=int),
'input_mask_1:0' : np.ones((batch, l), dtype=int),
}
        s = time.time()
        outs = sess.run(fetch, feed_dict = feed_dict)
        e = time.time()
