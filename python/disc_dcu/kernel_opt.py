...
def profile(tune, degree=50, cache=None):
    os.environ["DISC_KERNEL_PROFILING"] = "1"
    cmd = "{} --mode profile".format(get_path())
    if tune:
        cmd += " --tune --tunestep {}".format(degree)
    if cache is not None:
        cmd += " --cache {}".format(cache)
...
def optimize_kernel(tune=True, cache=None, limit=None, degree=50):
