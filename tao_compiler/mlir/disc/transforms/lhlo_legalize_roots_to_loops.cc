...
LogicalResult lowerWithScheduleColReductionForRocm(
  ...
  Value loop_limit =  b.create<arith::ConstantIndexOp>(loc, 64);
  
  Value var512 = b.create<arith::ConstantIndexOp>(loc, 512);
  Value var32 = b.create<arith::ConstantIndexOp>(loc, 32);
  Value block_limit = b.create<arith::ConstantIndexOp>(loc, 128);
  ...
  Value block_rows_bound = b.create<arith::CeilDivSIOp>(loc, block_limit, num_block_col);
  ...
  Value is_loop_gt = b.create<arith::CmpIOp>(loc, arith::CmpIPredicate::ugt,
                                        rows_loop, loop_limit);

  SmallVector<Value, 4> yield_values_for_gt;
  SmallVector<Type, 4> types_for_gt;
  for (int i = 0; i < 1; i++) {
    types_for_gt.push_back(var_rows.getType());
  }
  scf::IfOp if_loop_gt =
      b.create<scf::IfOp>(loc, /* resultTypes */ types_for_gt, is_loop_gt,
                      /*hasElseRegion*/ true);
  if_loop_gt.getThenRegion().front().clear();
  if_loop_gt.getElseRegion().front().clear();
  b.setInsertionPointToStart(&if_loop_gt.getThenRegion().front());
  b.create<scf::YieldOp>(loc, loop_base_var);
  b.setInsertionPointToStart(&if_loop_gt.getElseRegion().front());
  b.create<scf::YieldOp>(loc, rows_loop);
  b.setInsertionPointAfter(if_loop_gt);  
  var_loop_iter = b.create<arith::MulIOp>(loc, *(if_loop_gt.getResults().begin()), one);
  ...
  Value col_index;
  Value col_index_thread = b.create<arith::AddIOp>(
      loc, b.create<arith::MulIOp>(loc, block_col_index, var_tile_w),
      local_col_index);
  col_index = col_index_thread;
  ...
  Value row_index_loop = b.create<arith::AddIOp>(
      loc, b.create<arith::MulIOp>(loc, var_l, num_block_row),
      block_row_index);
...
template <int TILE_W, int TILE_H, int LOOP=8>
LogicalResult lowerWithScheduleColReductionForRocmVec(
    ArrayRef<Operation*> root_ops, Operation* dominant_op, Block* parent, int vec_size = 2,
    const ShapeAnalysis* shape_analysis = nullptr) {
  if (!isRank2ColReduction(dominant_op)) {
    return failure();
  }
  ...
  Value var_tile_h = b.create<arith::ConstantIndexOp>(loc, kTileH);
  Value var_tile_w = b.create<arith::ConstantIndexOp>(loc, kTileW);   
  Value var_vec_size = b.create<arith::ConstantIndexOp>(loc, vec_size);
  // if (TILE_H == 32) {
      // Value is_cols_gt512 = b.create<arith::CmpIOp>(loc, arith::CmpIPredicate::ule,
      //                                        var_cols, var512);
      // SmallVector<Value, 4> yield_values_for_gt512;
      ...
      // if_col_gt512.getElseRegion().front().clear();
      // b.setInsertionPointToStart(&if_col_gt512.getThenRegion().front());
  Value var_cols_vec2 = b.create<arith::CeilDivSIOp>(loc, var_cols, var_vec_size);
  Value num_block_col = b.create<arith::CeilDivSIOp>(loc, var_cols_vec2, var_tile_w);
  ...
  // VLOG(0) << "debug 0";
  SmallVector<AccumulatorFactory, 4> accum_factory(col_reduction_roots.size() * vec_size);
  // sum = init_value;
  SmallVector<Value, 4> init_values(col_reduction_roots.size() * vec_size);
  SmallVector<Type, 4> init_values_types(col_reduction_roots.size() * vec_size);
  for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
    Operation* root_op = root_pair.value();
    int idx = root_pair.index();
    Value init_value = b.create<memref::LoadOp>(
    ...
    for (int i = 0; i < vec_size; i++) {
      init_values[i * col_reduction_roots.size() + idx] = init_value;
      init_values_types[i * col_reduction_roots.size() + idx] = init_value.getType();
      accum_factory[i * col_reduction_roots.size() + idx] = std::move(getFactory(
          b, root_op->getLoc(), cast<lmhlo::ReduceOp>(root_op).body()));
    }
  }
  // local_row_index = n / var_tile_w;
  ...
  Value col_index_thread = b.create<arith::AddIOp>(
      loc, b.create<arith::MulIOp>(loc, block_col_index, var_tile_w),
      local_col_index);
  col_index = b.create<arith::MulIOp>(loc, col_index_thread, var_vec_size);

  // define SHM
  SmallVector<std::map<Operation*, Value>> shared_mem_map_vec(vec_size);
  // std::map<Operation*, Value> shared_mem_map;
  for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
    Operation* root_op = root_pair.value();
    int idx = root_pair.index();
  ...
  Value col_index_vec = col_index;

  for (int i = 0; i < vec_size; i++) {
    int col_idx = 0;
    SmallVector<Value, 2> multidim_load_index({row_index, col_index_vec});
    ValueRange load_index(multidim_load_index); 
    for (auto root_op : root_ops) {
      if (isRank2ColReduction(root_op)) {
        b.setInsertionPointToEnd(&if_row_valid.getThenRegion().front());
        ...
        AccumulatorFactory accumFactory =
            getFactory(b, root_op->getLoc(), reduce_op.body());
        auto acc = accumFactory(*(for_op_l.getRegionIterArgs().begin() + i * col_reduction_roots.size() + col_idx), data);
        yield_values_for_if.push_back(acc);
        col_idx++;
      }  else if (isRank2RowReduction(root_op)) {
        assert(false && "unexpected row_reduction");
    ...
    col_index_vec = b.create<arith::AddIOp>(loc, col_index_vec, one);
  }
  b.setInsertionPointToEnd(&if_row_valid.getThenRegion().front());
  b.create<scf::YieldOp>(loc, yield_values_for_if);
  b.setInsertionPointToEnd(&if_row_valid.getElseRegion().front());
  ...
  // Value var_n2 = b.create<arith::MulIOp>(loc, var_n, var_vec_size);
  // Value var_n2_1 = b.create<arith::AddIOp>(loc, var_n2, one);
  ...
  for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
      ...
      for (int i = 0; i < vec_size; i++) {
        b.create<memref::StoreOp>(loc,
                                *(for_op_l.getResults().begin() +  i * col_reduction_roots.size() + idx),
                                shared_mem_map_vec[i][root_op], var_n);
      }
      // b.create<memref::StoreOp>(loc,
      //                           *(for_op_l.getResults().begin() + idx * 2 + 1),
      //                           shared_mem_map[root_op], var_n2_1);
  }
  ...
  for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
    ...
    int idx = root_pair.index();
    for (int i = 0; i < vec_size; i++) {
      b.create<memref::StoreOp>(loc, init_values[i * col_reduction_roots.size() + idx], shared_mem_map_vec[i][root_op],
                                var_n);
    }
  }
  b.create<scf::YieldOp>(loc, ValueRange({}));
  ...
  for (int stride = kTileH / 2; stride > 1; stride /= 2) {
    ...
    Value var_shm_offset = b.create<mlir::arith::AddIOp>(
        loc, b.create<arith::MulIOp>(loc, var_stride, var_tile_w), var_n);
    // Value var_shm_offset2 = b.create<arith::MulIOp>(loc, var_shm_offset, two);
    // Value var_shm_offset2_1 = b.create<arith::AddIOp>(loc, var_shm_offset2, one);
    ...
    for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
      ...
      for (int i = 0; i < vec_size; i++) {
        Value shm_op0 =
            b.create<memref::LoadOp>(loc, shared_mem_map_vec[i][root_op], var_n);
        Value shm_op1 = b.create<memref::LoadOp>(loc, shared_mem_map_vec[i][root_op],
                                              var_shm_offset);
        b.create<memref::StoreOp>(loc, (accum_factory[i * col_reduction_roots.size() + idx])(shm_op0, shm_op1),
                                shared_mem_map_vec[i][root_op], var_n);
      }
    }
    b.create<scf::YieldOp>(loc, ValueRange({}));
  ...
  b.setInsertionPointToStart(&if_is_atom_add.getThenRegion().front());
  Value shm_offset = b.create<mlir::arith::AddIOp>(loc, var_tile_w, var_n);
  // Value shm_offset2 = b.create<mlir::arith::MulIOp>(loc, shm_offset, two);
  // Value shm_offset2_1 = b.create<mlir::arith::AddIOp>(loc, shm_offset2, one);
  
  for (auto root_pair : llvm::enumerate(col_reduction_roots)) {
    Operation* root_op = root_pair.value();
    int idx = root_pair.index();
    Value col_index_vec2 = col_index;
    for (int i = 0; i < vec_size; i++) {
      Value shm_op0 =
          b.create<memref::LoadOp>(loc, shared_mem_map_vec[i][root_op], var_n);
      Value shm_op1 =
          b.create<memref::LoadOp>(loc, shared_mem_map_vec[i][root_op], shm_offset);
      Value partial_result = (accum_factory[i * col_reduction_roots.size() + idx])(shm_op0, shm_op1);
      auto root_element_type = getLhloOpsElementType(root_op);
      b.create<memref::AtomicRMWOp>(
          loc, root_element_type,
          getAtomicRMWKind(cast<lmhlo::ReduceOp>(root_op).body()), partial_result,
          root_op->getOperand(2), ValueRange({col_index_vec2}));
      col_index_vec2 = b.create<arith::AddIOp>(loc, col_index_vec2, one);
    }
  }
  b.create<scf::YieldOp>(loc, ValueRange({}));
  ...
  cleanUnusedLhloOps(parent);
  llvm::errs() << "Finish rol reduction rocm vec2\n";
  return success();
  ...
  switch (fusion_type) {
    ...
    case FusionType::kColReduction: {
      ...
      if (col_reduction_schedule == DISC_TILE_W8_H32) {
        ...
        r = lowerWithScheduleColReductionForRocm<16, 32>(
            root_ops, dominant_op, fused_block); 
      } else if (col_reduction_schedule == DISC_TILE_LOOP_W32_H16) {
        r = lowerWithScheduleColReductionForRocm<32, 16>(
            root_ops, dominant_op, fused_block);
      } else if (col_reduction_schedule == DISC_TILE_LOOP_VEC2_W64_H8) {
        std::string vec2;
        static const char* env = getenv("VEC");
        if (env != nullptr) { 
          vec2 = std::string(env);
        }
        if (vec2 == "2") {
          r = lowerWithScheduleColReductionForRocmVec<64, 8>(
              root_ops, dominant_op, fused_block, 2);
        } else if (vec2 == "4") {
          r = lowerWithScheduleColReductionForRocmVec<64, 8>(
              root_ops, dominant_op, fused_block, 4);
        }else {
          r = lowerWithScheduleColReductionForRocm<64, 8>(
              root_ops, dominant_op, fused_block);
        }
